...
class EnvioEmailRepository:
    ...
    def serializar(envio: EnvioEmail) -> dict[str, Any]:
        """Converte um envio de e-mail em dicionário."""
        return EnvioEmailSerializer(envio).data

    @staticmethod
    def serializar_detalhe(envio: EnvioEmail) -> dict[str, Any]:
        """Converte um envio de e-mail com candidatos em dicionário."""
        return EnvioEmailDetalheSerializer(envio).data

    @classmethod
    def serializar_lista(cls, envios: list[EnvioEmail]) -> list[dict[str, Any]]:
        """Converte uma lista de envios em dicionários."""
        return EnvioEmailSerializer(envios, many=True).data

    @classmethod
    def listar_todos(cls) -> list[dict[str, Any]]:
        """Lista todos os envios de e-mail."""
        envios = list(
    ...
    def obter_por_uuid(cls, uuid: UUID) -> dict[str, Any] | None:
        """Retorna o envio pelo UUID ou None."""
        envio = (
    ...
    def criar(cls, **dados: Any) -> dict[str, Any]:
        """Cria um registro de envio de e-mail."""
...
class EnvioEmailCandidatoRepository:
    ...
    def serializar(candidato: EnvioEmailCandidato) -> dict[str, Any]:
        """Converte um candidato do envio em dicionário."""
    ...
    @classmethod
    def criar(cls, **dados: Any) -> dict[str, Any]:
        """Cria um candidato no envio de e-mail."""
        envio_email_uuid = dados.pop("envio_email")
        candidato = EnvioEmailCandidato.objects.create(**dados, envio_email_id=UUID(envio_email_uuid))
        return cls.serializar(candidato)

    @classmethod
    def obter_por_uuid(cls, uuid: UUID) -> dict[str, Any] | None:
        """Retorna o candidato pelo UUID ou None."""
    ...
    def atualizar_status(
        ...
        *,
        ...
        """Atualiza o status do candidato. Retorna False se não encontrado."""
        candidato = EnvioEmailCandidato.objects.filter(uuid=uuid).first()
        if candidato is None:
            return False
...
class EnvioEmailConteudoRepository:
    ...
    def serializar(conteudo: EnvioEmailConteudo) -> dict[str, Any]:
        """Converte um template de conteúdo em dicionário."""
    ...
    def carregar_instancia_por_uuid(
        ...
        """Carrega instância do template para operações de escrita."""
        return EnvioEmailConteudo.objects.filter(uuid=uuid).first()

    @classmethod
    def obter_por_uuid(cls, uuid: UUID) -> dict[str, Any] | None:
        """Retorna o template pelo UUID ou None."""
    ...
    def obter_por_tipo(cls, tipo: str) -> dict[str, Any] | None:
        """Retorna o template pelo tipo ou None."""
