...
from processos.models import EnvioEmailCandidato, ProcessoConvocacao
...
@patch(
    "processos.services.envio_email_service.CandidatosApiService.buscar_habilitados_por_processo"
)
...
def test_iniciar_processamento_envio_usa_assunto_informado(
    ...
    """Verifica envio com assunto customizado informado no payload."""
    mock_buscar.return_value = [
        {
            "candidato": {
                "nome": "Fulano",
...
def test_iniciar_processamento_envio_usa_gabarito_quando_conteudo_vazio(
    ...
    """Verifica fallback para conteudo gabarito quando payload de conteúdo vazio."""
    mock_buscar.return_value = [
        {
            "candidato": {
                "nome": "Fulano",
...
def test_resolver_conteudo_envio_usa_payload_quando_informado():
    """Verifica resolver conteudo envio usa payload quando informado."""
    assert _resolver_conteudo_envio(TIPO_CONVOCACAO, "<p>custom</p>") == "<p>custom</p>"


def test_resolver_conteudo_envio_fallback_para_gabarito(conteudo_convocacao):
    """Verifica fallback para conteudo gabarito quando payload vazio."""
    conteudo_convocacao.conteudo = ""
...
def test_resolver_assunto_envio_usa_payload_quando_informado():
    """Verifica resolver assunto envio usa payload quando informado."""
    assert _resolver_assunto_envio(TIPO_CONVOCACAO, "Assunto custom") == "Assunto custom"


def test_resolver_assunto_envio_fallback_para_padrao_do_tipo(conteudo_convocacao):
    """Verifica fallback para assunto padrão do tipo quando payload vazio."""
